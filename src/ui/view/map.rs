...
/// POI/GPS indicator circle radius at scale 1.
const INDICATOR_RADIUS: f32 = 5.;

/// POI/GPS indicator border size at scale 1.
const INDICATOR_BORDER: f32 = 2.;

/// Time after losing GPS signal before GPS indicator is removed.
const GPS_TIMEOUT: Duration = Duration::from_secs(10);

// TODO: Home button icon
// TODO: Maybe give GPS/POI a different map symbol?
//
/// Map rendering UI view.
pub struct MapView {
...
impl MapView {
    ...
    pub fn new(
        ...
        let size = Self::button_size(1.);
        let home_button = Button::new(point, size, Svg::Search);
        ...
        match point {
            ...
            None if self.gps_timeout.is_none() => {
                ...
                let token = self.event_loop.insert_source(timer, move |_, _, state| {
                    ...
                    map_view.dirty |= map_view.gps != None;
...
impl UiView for MapView {
    ...
    fn draw<'a>(&mut self, config: &Config, mut render_state: RenderState<'a>) {
        ...
        // Draw POI/GPS if visible.
        let points = self.poi.iter_mut().chain(&mut self.gps);
        for point in points {
            // Convert geographic to screen location.
            ...
            let fill_radius = INDICATOR_RADIUS * self.scale as f32;
            let border_radius = fill_radius + INDICATOR_BORDER * self.scale as f32;
            ...
            render_state.draw_circle(point, fill_radius, &self.tile_paint);
        }
